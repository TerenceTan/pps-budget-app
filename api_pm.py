...
def sync():
    ...
    def ensure_channel(country, quarter, mapped_channel):
        ...
        if USE_POSTGRES:
            pgdb.insert_channel(ch_id, country, quarter, mapped_channel, 0, sort_order, now)
        else:
    ...
    for row in rows_to_sync:
        ...
        entry_row = [entry_id, country, quarter, month, channel_id, mapped_channel, "", "", row.get("mapped_bu",""), row.get("mapped_finance_cat",""), row.get("mapped_marketing_cat",""), f"PM Sync: {channel_group}", 0, 0, round(spend,2), "", mapped_channel, f"Synced from BigQuery {now[:10]}", "False", "[]", "[]", session.get("username","pm_sync"), now, now]
        if USE_POSTGRES:
            pgdb.insert_entry(entry_row)
        else:
            ws_entries.append_row(entry_row)
        saved += 1
    if not USE_POSTGRES:
...
def auto_sync():
    try:
        ...
        if USE_POSTGRES:
            ...
            valid_countries = set(str(b.get("country","")) for b in pgdb.get_all(TAB_BUDGETS) if float(b.get("total_budget") or 0)>0) | set(MARKETS)
        else:
            ws_entries = get_sheet(TAB_ENTRIES); ws_channels = get_sheet(TAB_CHANNELS); ws_activities = get_sheet(TAB_ACTIVITIES)
        ...
        for agg_key, vals in sorted(agg.items()):
            ...
            if ch: channel_id = str(ch["id"])
                ...
                if USE_POSTGRES:
                    pgdb.insert_channel(channel_id,country,quarter,mapped_channel,0,so,now)
                else:
                    ws_channels.append_row([channel_id,country,quarter,mapped_channel,0,so,now])
                existing_channels.append({"id":channel_id,"country":country,"quarter":quarter,"name":mapped_channel,"budget":0,"sort_order":so}); channels_created+=1
            ...
            if act: activity_id = str(act["id"])
                ...
                if USE_POSTGRES:
                    pgdb.insert_activity(activity_id,channel_id,country,quarter,activity_name,so,now)
                else:
                    ws_activities.append_row([activity_id,channel_id,country,quarter,activity_name,so,now])
                existing_activities.append({"id":activity_id,"channel_id":channel_id,"country":country,"quarter":quarter,"name":activity_name,"sort_order":so}); activities_created+=1
            if existing:
                ...
                try:
                    ...
                    row_vals = [str(e.get("country","")), str(e.get("quarter","")), str(e.get("month","")),
                        ...
                        0, float(e.get("confirmed") or 0), new_actual,
                    ...
                    updated+=1
                except Exception as ue: print(f"Auto-sync error: {ue}"); skipped+=1
                continue
            if spend<=0: skipped+=1; continue
            entry_id = "pm_"+str(uuid.uuid4())[:10]
            entry_row = [entry_id,country,quarter,month_key,channel_id,mapped_channel,activity_id,activity_name,mapped_bu,mapped_finance_cat,mapped_marketing_cat,f"{channel_group}: ${spend:,.0f} AUD",0,0,spend,"",mapped_channel,f"Auto-synced {now[:10]}","False","[]","[]",session.get("username","pm_auto"),now,now]
            if USE_POSTGRES:
                pgdb.insert_entry(entry_row)
            else:
                ws_entries.append_row(entry_row)
