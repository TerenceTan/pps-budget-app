...
def api_get_budget(country, quarter):
    ...
    brows = rows_for(TAB_BUDGETS, country=country, quarter=quarter)
    ...
    channels = sorted([
        ...
        for r in rows_for(TAB_CHANNELS, country=country, quarter=quarter)
    ], key=lambda x: x["sort_order"])

    # Attach activities to each channel
    all_acts = rows_for(TAB_ACTIVITIES, country=country, quarter=quarter)
    for ch in channels:
    ...
    try:
        mrows = get_sheet(TAB_MAPPING).get_all_records()
        if not mrows:
            _seed_mapping()
            mrows = get_sheet(TAB_MAPPING).get_all_records()
...
def api_get_entries(country, quarter):
    if not check_country_access(country):
        return jsonify({"error":"Forbidden"}), 403
    rows = rows_for(TAB_ENTRIES, country=country, quarter=quarter)
    entries = []
    for r in rows:
        entries.append({
...
def api_add_entry():
    ...
    get_sheet(TAB_ENTRIES).append_row([
        ...
        json.dumps(d.get("invoice_names",[])),
        json.dumps(d.get("invoice_data",[])),
...
def api_update_entry(entry_id):
    ...
    inv_names = json.dumps(d.get("invoice_names", json.loads(r.get("invoice_names") or "[]")))
    inv_data  = json.dumps(d.get("invoice_data",  json.loads(r.get("invoice_data")  or "[]")))
    now = datetime.utcnow().isoformat()
...
def api_invoice(entry_id, inv_idx):
    ...
    header, b64 = datas[inv_idx].split(",", 1)
...
def api_channel_template():
    ...
    # Example rows covering all markets + quarters
    examples = [
        ('TH','Q1','Performance Marketing',50000),
        ('TH','Q1','Affiliate',30000),
        ('TH','Q1','Paid Social',25000),
        ('TH','Q2','Performance Marketing',55000),
        ('TH','Q2','Affiliate',32000),
        ('SG','Q1','Performance Marketing',40000),
        ('SG','Q1','Paid Social',20000),
        ('MY','Q1','Performance Marketing',35000),
        ('MY','Q1','Affiliate',18000),
    ]
    for row in examples:
        w.writerow(row)
    return send_file(
...
# ── COUNTRY BUDGET TEMPLATE DOWNLOAD ─────────────────────────
...
def api_budget_template():
    ...
    markets = ["CN","HKG","ID","IN","MN","MY","PH","SG","TH","TW","VN"]
    quarters = ["Q1","Q2","Q3","Q4"]
    for mkt in markets:
        for q in quarters:
            w.writerow([mkt, q, ''])   # admin fills in the amounts
    return send_file(
        io.BytesIO(out.getvalue().encode('utf-8-sig')),
        mimetype='text/csv', as_attachment=True,
...
def api_import_channels():
    """
    Accepts CSV with columns: Country, Quarter, Channel Name, Budget
    ...
    for row in rows:
        if len(row) < 3:
            skipped += 1
            continue
        # Support both 4-col (Country, Quarter, Name, Budget) and 3-col (Quarter, Name, Budget)
        if len(row) >= 4:
            country_val = row[0].strip()
            quarter_val = row[1].strip().upper()
            name_val    = row[2].strip()
            budget_val  = float(row[3].replace(',','').replace('$','').strip() or 0) if row[3].strip() else 0
        else:
            country_val = request.form.get('country', '')
            quarter_val = row[0].strip().upper()
            name_val    = row[1].strip()
            budget_val  = float(row[2].replace(',','').replace('$','').strip() or 0) if row[2].strip() else 0

        if not country_val or not quarter_val or not name_val:
            skipped += 1
            continue

        # Normalise quarter (accept Q1/q1/1)
...
def api_import_budgets():
    """
    Accepts CSV with columns: Country, Quarter, Total Budget
